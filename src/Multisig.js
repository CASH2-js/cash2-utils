...
class Multisig {
    ...
    get address() {
        ...
        return Address_1.Address.fromViewOnlyKeys(this.spend, this.view);
    }
    /**
    ...
    get calculated_multisig_keys() {
        if (this.m_threshold !== this.m_participants) {
            ...
            this.m_wallet_multisig_keys.forEach((multisig_key) => {
                const keys = Types_1.TurtleCoinCrypto.calculateMultisigPrivateKeys(multisig_key.privateKey, this.m_participant_keys);
                keys.forEach((key) => {
    ...
    static fromMultisigKeys(multisig_private_keys, sharedPrivateViewKey, threshold, participants) {
        ...
        multisig_private_keys.forEach((key) => {
            if (!Types_1.TurtleCoinCrypto.checkScalar(key)) {
