...
const turtlecoin_base58_1 = require("cash2-base58");
...
class Address {
    ...
    static fromAddress(address, prefix) {
        ...
        const checksum = (new bytestream_helper_1.Reader(Types_1.TurtleCoinCrypto.cn_fast_hash(decodedPrefix + paymentId + publicSpend + publicView))).bytes(SIZES.CHECKSUM).toString('hex');
        if (expectedChecksum !== checksum) {
            throw new Error('Could not parse address: checksum mismatch');
        }
        const result = new Address();
        result.m_paymentId = paymentId;
        result.m_keys = new Types_1.ED25519.Keys(new Types_1.ED25519.KeyPair(publicSpend), new Types_1.ED25519.KeyPair(publicView));
        return result;
    }
    /**
    ...
    static fromPublicKeys(publicSpendKey, publicViewKey, paymentId, prefix) {
        ...
        address.m_keys = new Types_1.ED25519.Keys(new Types_1.ED25519.KeyPair(publicSpendKey), new Types_1.ED25519.KeyPair(publicViewKey));
        return address;
    }
    /**
    ...
    static fromViewOnlyKeys(publicSpendKey, privateViewKey, paymentId, prefix) {
        ...
        address.m_keys = new Types_1.ED25519.Keys(new Types_1.ED25519.KeyPair(publicSpendKey), new Types_1.ED25519.KeyPair(undefined, privateViewKey));
        return address;
    }
    /**
    ...
    static fromKeys(privateSpendKey, privateViewKey, prefix) {
        ...
        address.m_keys = new Types_1.ED25519.Keys(new Types_1.ED25519.KeyPair(undefined, privateSpendKey), new Types_1.ED25519.KeyPair(undefined, privateViewKey));
        const derivedViewKey = new Types_1.ED25519.KeyPair(undefined, privateSpendKey, undefined, 1);
    ...
    static fromSeed(seed, language, prefix) {
        ...
        if (!Common_1.Common.isHex64(seed)) {
            seed = Types_1.TurtleCoinCrypto.cn_fast_hash(seed);
        }
        const address = new Address();
        address.m_seed = seed;
        ...
        address.m_keys = new Types_1.ED25519.Keys(new Types_1.ED25519.KeyPair(undefined, seed), new Types_1.ED25519.KeyPair(undefined, seed, undefined, 1));
        return address;
    }
    /**
    ...
    static generateSeed(entropy, iterations) {
        const random = new Types_1.ED25519.KeyPair(undefined, undefined, entropy, iterations);
    ...
    static generateSubwallet(privateSpendKey, subwalletIndex, language, prefix) {
        ...
        if (!Types_1.TurtleCoinCrypto.checkScalar(privateSpendKey)) {
        ...
        const view = new Types_1.ED25519.KeyPair(undefined, privateSpendKey, undefined, 1);
        const spend = Types_1.TurtleCoinCrypto.generateDeterministicSubwalletKeys(privateSpendKey, subwalletIndex);
        address.m_keys = new Types_1.ED25519.Keys(new Types_1.ED25519.KeyPair(spend.publicKey, spend.privateKey), view);
        return address;
    }
    /**
    ...
    toString() {
        ...
        const checksum = Types_1.TurtleCoinCrypto.cn_fast_hash(writer.blob).slice(0, 8);
