...
class Block {
    ...
    get transactionTreeHash() {
        const transactions = [this.m_minerTransaction.hash].concat(this.transactions);
        const treeHash = Types_1.TurtleCoinCrypto.tree_hash(transactions);
    ...
    static from(data, config) {
        ...
        if (block.m_majorVersion >= activateParentBlockVersion) {
            ...
            const baseTransactionBranchDepth = Types_1.TurtleCoinCrypto.tree_depth(block.m_parentBlock.transactionCount);
    ...
    toHashingBuffer(headerOnly = false) {
        ...
        if (this.m_majorVersion >= this.m_activateParentBlockVersion) {
            ...
            writer.hash(this.m_parentBlock.previousBlockHash);
            writer.uint32_t(this.m_nonce, true);
            const treeHash = Types_1.TurtleCoinCrypto.tree_hash_from_branch(this.m_parentBlock.baseTransactionBranch, this.m_parentBlock.minerTransaction.hash, 0);
...
function getBlockPoWHash(data, majorVersion) {
    ...
    switch (majorVersion) {
        ...
        case 6:
            return Types_1.TurtleCoinCrypto.chukwa_slow_hash(blob);
        default:
