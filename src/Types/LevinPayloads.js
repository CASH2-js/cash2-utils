...
(function (LevinPayloads) {
    ...
    class LiteBlock {
        ...
        static from(data) {
            ...
            result.blockTemplate = Block_1.Block.from(payload.get('blockTemplate'));
            return result;
        }
        /**
         * Provides the Buffer representation of the object
    ...
    class NewBlock {
        ...
        static from(data) {
            ...
            if (payload.exists('block')) {
                ...
                result.block = Block_1.Block.from(blockPayload.get('block'));
                if (blockPayload.exists('txs')) {
                    blockPayload.get('txs').forEach((tx) => {
                        result.transactions.push(Transaction_1.Transaction.from(tx));
                    });
                }
            }
            result.current_blockchain_height =
    ...
    class NewTransactions {
        ...
        static from(data) {
            ...
            if (payload.exists('txs')) {
                payload.get('txs')
                    .forEach((tx) => result.transactions.push(Transaction_1.Transaction.from(tx)));
            }
            return result;
        }
        /**
    ...
    class ResponseGetObjects {
        ...
        static from(data) {
            ...
            if (payload.exists('blocks')) {
                payload.get('blocks')
                    .forEach((block) => {
                    const l_block = Block_1.Block.from(block.get('block'));
                    const l_txs = [];
                    if (block.exists('txs')) {
                        block.get('txs')
                            .forEach((tx) => {
                            l_txs.push(Transaction_1.Transaction.from(tx));
                        });
                    }
