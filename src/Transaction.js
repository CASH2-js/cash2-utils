...
class Transaction {
     ...
     */
    get amount() {
        const amount = Types_1.BigInteger.zero;
     ...
     */
    get fee() {
        const inputAmount = Types_1.BigInteger(this.amount);
        if (inputAmount === Types_1.BigInteger.zero) {
            return 0;
        }
        const outputAmount = Types_1.BigInteger.zero;
    ...
    get hash() {
        ...
        const hash = Types_1.TurtleCoinCrypto.cn_fast_hash(this.m_cached.blob);
        if (this.version >= 2) {
            const hash2 = Types_1.TurtleCoinCrypto.cn_fast_hash(hash + TransactionVersion2Suffix);
    ...
    get prefixHash() {
        ...
        const hash = Types_1.TurtleCoinCrypto.cn_fast_hash(this.m_cached.prefix);
        if (this.version >= 2) {
            const hash2 = Types_1.TurtleCoinCrypto.cn_fast_hash(hash + TransactionVersion2Suffix);
