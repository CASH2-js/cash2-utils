...
export class Multisig {
     ...
     */
    get calculated_multisig_keys() {
        if (this.m_threshold !== this.m_participants) {
            const multisig_keys = [];
            this.m_wallet_multisig_keys.forEach((multisig_key) => {
                const keys = TurtleCoinCrypto.calculateMultisigPrivateKeys(multisig_key.privateKey, this.m_participant_keys);
                keys.forEach((key) => {
...
function isValidThreshold(threshold, participants) {
    return (!((threshold / participants) <= .5) && required_keys(threshold, participants) <= Math.pow(2, 13));
}
/** @ignore */
...
function generatePartialSigningKey(preparedSignature, index, privateSpendKey) {
    return __awaiter(this, void 0, void 0, function* () {
        const key = yield TurtleCoinCrypto.generatePartialSigningKey(preparedSignature, privateSpendKey);
        return {
            key,
            index,
        };
...
function restoreRingSignatures(derivation, outputIndex, partialSigningKeys, realOutputIndex, key, signatures, index) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const sigs = yield TurtleCoinCrypto.restoreRingSignatures(derivation, outputIndex, keys, realOutputIndex, key, signatures);
        return {
            sigs,
            index,
        };
