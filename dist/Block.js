...
export class Block {
    constructor() {
        ...
        this.m_parentBlock = new ParentBlock();
        this.m_nonce = 0;
        this.m_minerTransaction = new Transaction();
        this.m_transactions = [];
    ...
    static from(data, config) {
        ...
        if (block.m_majorVersion >= activateParentBlockVersion) {
            ...
            for (let i = 0; i < p_inputs; i++) {
                if (reader.uint8_t().toJSNumber() === TransactionInputs.InputType.COINBASE) {
                    block.m_parentBlock.minerTransaction.inputs.push(new TransactionInputs.CoinbaseInput(reader.varint().toJSNumber()));
                }
                else {
            ...
            for (let i = 0; i < p_outputs; i++) {
                ...
                if (type === TransactionOutputs.OutputType.KEY) {
                    block.m_parentBlock.minerTransaction.outputs.push(new TransactionOutputs.KeyOutput(amount, reader.hash()));
                }
                else {
        ...
        for (let i = 0; i < inputs; i++) {
            if (reader.uint8_t().toJSNumber() === TransactionInputs.InputType.COINBASE) {
                block.m_minerTransaction.inputs.push(new TransactionInputs.CoinbaseInput(reader.varint().toJSNumber()));
            }
            else {
        ...
        for (let i = 0; i < outputs; i++) {
            ...
            const type = reader.uint8_t().toJSNumber();
            if (type === TransactionOutputs.OutputType.KEY) {
                block.m_minerTransaction.outputs.push(new TransactionOutputs.KeyOutput(amount, reader.hash()));
            }
            else {
                throw new Error('Unknown output type detected');
            }
    ...
    toHashingBuffer(headerOnly = false) {
        ...
        if (this.m_majorVersion >= this.m_activateParentBlockVersion) {
            ...
            writer.hash(this.m_parentBlock.previousBlockHash);
            writer.uint32_t(this.m_nonce, true);
            const treeHash = TurtleCoinCrypto.tree_hash_from_branch(this.m_parentBlock.baseTransactionBranch, this.m_parentBlock.minerTransaction.hash, 0);
