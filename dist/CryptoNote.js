...
import { BigInteger, ED25519, TransactionInputs, TransactionOutputs, TurtleCoinCrypto } from './Types';
...
export class CryptoNote {
    ...
    generateKeyImage(transactionPublicKey, privateViewKey, publicSpendKey, privateSpendKey, outputIndex) {
        return __awaiter(this, void 0, void 0, function* () {
            const derivation = yield TurtleCoinCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey);
    ...
    generateKeyImagePrimitive(publicSpendKey, privateSpendKey, outputIndex, derivation) {
        return __awaiter(this, void 0, void 0, function* () {
            const publicEphemeral = yield TurtleCoinCrypto.derivePublicKey(derivation, outputIndex, publicSpendKey);
            const privateEphemeral = yield TurtleCoinCrypto.deriveSecretKey(derivation, outputIndex, privateSpendKey);
    ...
    isOurTransactionOutput(transactionPublicKey, output, privateViewKey, publicSpendKey, privateSpendKey, generatePartial) {
        return __awaiter(this, void 0, void 0, function* () {
            const derivedKey = yield TurtleCoinCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey);
            const publicEphemeral = yield TurtleCoinCrypto.derivePublicKey(derivedKey, output.index, publicSpendKey);
            if (publicEphemeral === output.key) {
                ...
                if (privateSpendKey) {
                    ...
                    const privateEphemeral = (generatePartial) ?
                        privateSpendKey :
                        yield TurtleCoinCrypto.deriveSecretKey(derivedKey, output.index, privateSpendKey);
                    const derivedPublicEphemeral = yield TurtleCoinCrypto.secretKeyToPublicKey(privateEphemeral);
                    if (derivedPublicEphemeral !== publicEphemeral && !generatePartial) {
                        throw new Error('Incorrect private spend key supplied');
                    }
                    const keyImage = yield TurtleCoinCrypto.generateKeyImage(publicEphemeral, privateEphemeral);
    ...
    signMessage(message, privateKey) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const publicKey = yield TurtleCoinCrypto.secretKeyToPublicKey(privateKey);
            const hex = Buffer.from(message);
            const hash = yield TurtleCoinCrypto.cn_fast_hash(hex.toString('hex'));
    ...
    verifyMessageSignature(message, publicKey, signature) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const hex = Buffer.from(message);
            const hash = yield TurtleCoinCrypto.cn_fast_hash(hex.toString('hex'));
            const valid = yield TurtleCoinCrypto.checkSignature(hash, publicKey, signature);
            if (!valid) {
     ...
     */
    prepareTransaction(outputs, inputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extraData) {
        return __awaiter(this, void 0, void 0, function* () {
            const feePerByte = this.config.activateFeePerByteTransactions || Config.activateFeePerByteTransactions || false;
            const prepared = yield this.createTransactionStructure(outputs, inputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extraData);
            const recipients = [];
            ...
            for (let i = 0; i < prepared.inputs.length; i++) {
                ...
                const srcKeys = [];
                input.outputs.forEach((out) => srcKeys.push(out.key));
                promises.push(prepareRingSignatures(txPrefixHash, input.keyImage, srcKeys, input.realOutputIndex, input.input.transactionKeys.derivedKey, input.input.transactionKeys.outputIndex, i));
            }
            const results = yield Promise.all(promises);
...
function generateRingSignatures(hash, keyImage, publicKeys, privateKey, realOutputIndex, index) {
    return __awaiter(this, void 0, void 0, function* () {
        const signatures = yield TurtleCoinCrypto.generateRingSignatures(hash, keyImage, publicKeys, privateKey, realOutputIndex);
...
/** @ignore */
function prepareRingSignatures(hash, keyImage, publicKeys, realOutputIndex, derivation, outputIndex, index) {
    return __awaiter(this, void 0, void 0, function* () {
        const prepped = yield TurtleCoinCrypto.prepareRingSignatures(hash, keyImage, publicKeys, realOutputIndex);
        return {
            index: index,
...
function completeRingSignatures(privateSpendKey, derivation, outputIndex, realOutputIndex, key, sigs, index) {
    return __awaiter(this, void 0, void 0, function* () {
        const privateEphemeral = yield TurtleCoinCrypto.deriveSecretKey(derivation, outputIndex, privateSpendKey);
        const signatures = yield TurtleCoinCrypto.completeRingSignatures(privateEphemeral, realOutputIndex, key, sigs);
        return { signatures, index };
    });
}
/** @ignore */
...
function prepareTransactionOutputs(outputs) {
    return __awaiter(this, void 0, void 0, function* () {
        function prepareOutput(destination, amount, index, privateKey) {
            return __awaiter(this, void 0, void 0, function* () {
                const outDerivation = yield TurtleCoinCrypto.generateKeyDerivation(destination.view.publicKey, privateKey);
                const outPublicEphemeral = yield TurtleCoinCrypto.derivePublicKey(outDerivation, index, destination.spend.publicKey);
                return {
                    amount,
        ...
        const transactionKeys = new ED25519.KeyPair(keys.publicKey, keys.privateKey);
